...
tools to visualize and analyze spatial inequality. Methods such as local
spatial autocorrelation analysis \citep{anselin1995LocalIndicators},
spatial distribution dynamics \citep{rey2015DiscreteRegionala}, and
regionalization \citep{wei2020EfficientRegionalization} offer robust
frameworks to identify patterns, relationships, and variations across
geographical contexts. Thus, spatial data science is well-positioned to
support such interdisciplinary research. The goal of this chapter is to
introduce social scientists to the concepts and measurement of spatial
inequality. The emphasis is on adopting a computationally focused and
reproducible treatment that would allow researchers to apply the methods
introduced here to their own investigations.

The focus is on a case study of regional income inequality in Mexico,
which provides a compelling example of spatial disparities and their
implications for both scientific inquiry and policy-making. Mexico's
diverse regional landscape, marked by significant economic, social, and
cultural differences, offers a rich context for analyzing how inequality
manifests across geographical spaces. By examining income inequality at
the regional level, this case study highlights the spatial patterns and
clusters of economic disparities, providing insight into the underlying
processes that drive inequality.
...
\citep{suss2024GEOWEALTHUSSpatial}.\footnote{It is important to keep in
  ...
  variables are examined. See the discussions in
  \citep{saez2016WealthInequality} and
  \citep{piketty2014CapitalTwentyfirst}.} More granular studies examine
disparities in the sources of income, such as wages, as well as
employment rates \citep{overman2022SpatialDisparities}. Outside of
economics, topics such as disparities in educational outcomes
...
\citep{u.s.departmentofcommerce2023GeographicInequality, sarkar2024SpatialInequalities},
and down to neighborhoods \citep{nijman2020UrbanInequalities}.
...
\ImportTok{import}\NormalTok{ watermark }\hspace*{\fill}\NormalTok{\circled{3}}
...
\item[\circled{3}]
\texttt{watermark} reports the current versions of our packages to
...
Last updated: Tue Mar 04 2025 11:04:28
...
scipy     : 1.14.1
matplotlib: 3.9.3
inequality: 0.1.dev204+g6d702d5
seaborn   : 0.13.2
libpysal  : 4.12.1
\end{verbatim}
...
state per capita gross domestic product (pcgdp) measured in 2000 USD
measured for each decade from 1940-2000 for each of 32 areas consisting
of the 31 federal states of Mexico plus Mexico City. This data-set is
included as an example data-set in \texttt{libpysal}.
...
The following code block produces Figure~\ref{fig-states} which lists
the locations of the 32 Mexican states.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Create a map to provide}
\CommentTok{\# context for the subsequent analysis}

\CommentTok{\# We use the library mapclassify for choropleth classifications}
\ImportTok{import}\NormalTok{ mapclassify}

\CommentTok{\# set the path to our shapefile and read the file into a geodatafame}
...
\CommentTok{\# we will use greedy from mapclassify}
\CommentTok{\# states to ensure contiguous states are of a different color}
\NormalTok{sgdf }\OperatorTok{=}\NormalTok{ gdf.sort\_values(by}\OperatorTok{=}\StringTok{\textquotesingle{}NAME\textquotesingle{}}\NormalTok{)}
\NormalTok{sgdf.reset\_index(inplace}\OperatorTok{=}\VariableTok{True}\NormalTok{)}
\NormalTok{sgdf[}\StringTok{\textquotesingle{}label\textquotesingle{}}\NormalTok{] }\OperatorTok{=} \BuiltInTok{range}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{33}\NormalTok{)}
\NormalTok{sgdf[}\StringTok{\textquotesingle{}greedy\textquotesingle{}}\NormalTok{] }\OperatorTok{=}\NormalTok{ mapclassify.greedy(sgdf)}

\CommentTok{\# set the font size and position}
\NormalTok{font\_size }\OperatorTok{=} \DecValTok{9}
\NormalTok{outside }\OperatorTok{=}\NormalTok{ [}\DecValTok{9}\NormalTok{, }\DecValTok{29}\NormalTok{]}
\NormalTok{oc }\OperatorTok{=}\NormalTok{ [(}\OperatorTok{{-}}\DecValTok{103}\NormalTok{, }\FloatTok{17.5}\NormalTok{), (}\OperatorTok{{-}}\DecValTok{95}\NormalTok{, }\FloatTok{22.5}\NormalTok{)]}
\NormalTok{oe }\OperatorTok{=}\NormalTok{ [(}\OperatorTok{{-}}\FloatTok{102.55}\NormalTok{, }\FloatTok{17.49}\NormalTok{),(}\OperatorTok{{-}}\FloatTok{95.5}\NormalTok{, }\FloatTok{22.1}\NormalTok{)] }
\NormalTok{oinfo }\OperatorTok{=} \BuiltInTok{zip}\NormalTok{(outside, oc)}

\CommentTok{\# we will use LineStrings from shapely}
\CommentTok{\# for call{-}outs to state names}
\ImportTok{from}\NormalTok{ shapely.geometry }\ImportTok{import}\NormalTok{ LineString}

\CommentTok{\# plot the map }
\ImportTok{import}\NormalTok{ matplotlib.pyplot }\ImportTok{as}\NormalTok{ plt}
\NormalTok{sgdf[}\StringTok{\textquotesingle{}centroid\textquotesingle{}}\NormalTok{] }\OperatorTok{=}\NormalTok{ sgdf.centroid}
\NormalTok{ax }\OperatorTok{=}\NormalTok{ sgdf.plot(}
\NormalTok{    figsize}\OperatorTok{=}\NormalTok{(}\DecValTok{8}\NormalTok{, }\DecValTok{12}\NormalTok{),}
\NormalTok{    column}\OperatorTok{=}\StringTok{"greedy"}\NormalTok{,}
\NormalTok{    categorical}\OperatorTok{=}\VariableTok{True}\NormalTok{,}
\NormalTok{    cmap}\OperatorTok{=}\StringTok{"Set3"}\NormalTok{,}
    \CommentTok{\#legend=True,}
\NormalTok{    edgecolor}\OperatorTok{=}\StringTok{"w"}\NormalTok{,}
\NormalTok{)}

\CommentTok{\# build the table of state names}
\NormalTok{table }\OperatorTok{=}\NormalTok{ []}
\ControlFlowTok{for}\NormalTok{ idx, row }\KeywordTok{in}\NormalTok{ sgdf.iterrows():}
\NormalTok{    centroid }\OperatorTok{=}\NormalTok{ row[}\StringTok{\textquotesingle{}centroid\textquotesingle{}}\NormalTok{]}
\NormalTok{    table.append(}\SpecialStringTok{f\textquotesingle{}}\SpecialCharTok{\{}\NormalTok{idx}\OperatorTok{+}\DecValTok{1}\SpecialCharTok{:2d\}}\SpecialStringTok{ }\SpecialCharTok{\{}\NormalTok{row[}\StringTok{"NAME"}\NormalTok{]}\SpecialCharTok{\}}\SpecialStringTok{\textquotesingle{}}\NormalTok{)}
    \ControlFlowTok{if}\NormalTok{ idx}\OperatorTok{+}\DecValTok{1} \KeywordTok{not} \KeywordTok{in}\NormalTok{ outside:}
\NormalTok{        ax.text(centroid.x, centroid.y, }\BuiltInTok{str}\NormalTok{(idx}\OperatorTok{+}\DecValTok{1}\NormalTok{), ha}\OperatorTok{=}\StringTok{\textquotesingle{}center\textquotesingle{}}\NormalTok{,}
\NormalTok{                va}\OperatorTok{=}\StringTok{\textquotesingle{}center\textquotesingle{}}\NormalTok{, fontsize}\OperatorTok{=}\NormalTok{font\_size, color}\OperatorTok{=}\StringTok{\textquotesingle{}black\textquotesingle{}}\NormalTok{)}

\CommentTok{\# add the call{-}outs and number the polygons}
\NormalTok{i }\OperatorTok{=} \DecValTok{0}
\ControlFlowTok{for}\NormalTok{ out }\KeywordTok{in}\NormalTok{ oinfo:}
\NormalTok{    idx, coords }\OperatorTok{=}\NormalTok{ out}
\NormalTok{    ax.text(coords[}\DecValTok{0}\NormalTok{], coords[}\DecValTok{1}\NormalTok{], }\BuiltInTok{str}\NormalTok{(idx), ha}\OperatorTok{=}\StringTok{\textquotesingle{}center\textquotesingle{}}\NormalTok{,}
\NormalTok{            va}\OperatorTok{=}\StringTok{\textquotesingle{}center\textquotesingle{}}\NormalTok{, fontsize}\OperatorTok{=}\NormalTok{font\_size, color}\OperatorTok{=}\StringTok{\textquotesingle{}black\textquotesingle{}}\NormalTok{)}
\NormalTok{    start\_point }\OperatorTok{=}\NormalTok{ coords}
\NormalTok{    end\_point }\OperatorTok{=}\NormalTok{ sgdf.centroid[idx}\OperatorTok{{-}}\DecValTok{1}\NormalTok{]}

    \CommentTok{\# Create a LineString object}
\NormalTok{    start\_point }\OperatorTok{=}\NormalTok{ oe[i]}
\NormalTok{    line }\OperatorTok{=}\NormalTok{ LineString([start\_point, end\_point])}
    
    \CommentTok{\# Create a GeoSeries for the line}
\NormalTok{    line\_gdf }\OperatorTok{=}\NormalTok{ gpd.GeoSeries([line])}
    
    \CommentTok{\# Plot the line}
\NormalTok{    line\_gdf.plot(ax}\OperatorTok{=}\NormalTok{ax, color}\OperatorTok{=}\StringTok{\textquotesingle{}red\textquotesingle{}}\NormalTok{, linewidth}\OperatorTok{=}\DecValTok{2}\NormalTok{)}
\NormalTok{    i}\OperatorTok{+=}\DecValTok{1}

\ControlFlowTok{for}\NormalTok{ i, label }\KeywordTok{in} \BuiltInTok{enumerate}\NormalTok{(table):}
    \ControlFlowTok{if}\NormalTok{ i }\OperatorTok{\textless{}} \DecValTok{16}\NormalTok{:}
\NormalTok{        ax.text(}\OperatorTok{{-}}\DecValTok{120}\NormalTok{, }\DecValTok{20}\OperatorTok{{-}}\NormalTok{i}\OperatorTok{*}\DecValTok{1}\NormalTok{, label, ha}\OperatorTok{=}\StringTok{\textquotesingle{}left\textquotesingle{}}\NormalTok{,}
\NormalTok{                va}\OperatorTok{=}\StringTok{\textquotesingle{}center\textquotesingle{}}\NormalTok{, fontsize}\OperatorTok{=}\NormalTok{font\_size, color}\OperatorTok{=}\StringTok{\textquotesingle{}black\textquotesingle{}}\NormalTok{)}\OperatorTok{;}
    \ControlFlowTok{else}\NormalTok{:}
\NormalTok{        ax.text(}\OperatorTok{{-}}\DecValTok{110}\NormalTok{, }\DecValTok{20}\OperatorTok{{-}}\NormalTok{(i}\OperatorTok{{-}}\DecValTok{16}\NormalTok{)}\OperatorTok{*}\DecValTok{1}\NormalTok{, label, ha}\OperatorTok{=}\StringTok{\textquotesingle{}left\textquotesingle{}}\NormalTok{,}
\NormalTok{                va}\OperatorTok{=}\StringTok{\textquotesingle{}center\textquotesingle{}}\NormalTok{, fontsize}\OperatorTok{=}\NormalTok{font\_size, color}\OperatorTok{=}\StringTok{\textquotesingle{}black\textquotesingle{}}\NormalTok{)}\OperatorTok{;}
...
for 1990, together with the state name, and five variables that define
different regionalization schemes for the country. We will return to
these regional variables in a later section.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Create a DataFrame from a csv file}
...
incomes in Mexico. There are two different approaches towards
visualizing the distribution: one focusing on the geographical
distribution and the second on the attribute distribution. From a
geographical perspective, Figure~\ref{fig-mappcgdp2000} shows the
spatial distribution of incomes for the last year of the sample (2000).

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Create a choropleth map of per capita gross}
\CommentTok{\# domestic product in 2000 using quintiles for the classification}
\NormalTok{pth }\OperatorTok{=}\NormalTok{ lps.examples.get\_path(}\StringTok{"mexicojoin.shp"}\NormalTok{)}
\NormalTok{gdf }\OperatorTok{=}\NormalTok{ gpd.read\_file(pth)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Produce the Pen\textquotesingle{}s Parade for 2000}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Produce pengram for 2000}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Code snippet to produce the pengram}
\CommentTok{\# with query for Chiapas and Distrito Federal}
\ImportTok{from}\NormalTok{ inequality.pen }\ImportTok{import}\NormalTok{ pengram}
...
together with a kernel density estimate, and a rug plot. The kernel
density estimate is a smooth curve that represents the probability
density function of the data, providing a continuous approximation of
the underlying distribution. The rug plot signifies the positions of
each state as short ticks on the x-axis. The outlier nature of the
Distrito Federal that we saw in the \texttt{pengram} is responsible for
the positive (right) skew of the density function. There is some
evidence of polarization in the distribution with the mode being at the
poorest group of states and other, lower, peaks in the middle of the
distribution.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Produce histogram,}
\CommentTok{\# kernel density and rug plot}
...
Product by State 2000 (pcgdp2000), histogram, kernel density, rug plot}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Produce Lorenz Curve and Schutz Line}
...
A large number of measures of inequality exist, and choosing among the
rich diversity of inequality measures has been the subject of a vast
literature.\footnote{See \citet{cowell2011MeasuringInequality} for an
  overview of inequality measures.} To help guide that selection, there
are five desirable properties of an inequality measure:
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Code snippet to calculate inequality measures for each decade}
\CommentTok{\# and display in a table}
\NormalTok{yvars }\OperatorTok{=}\NormalTok{ [}\SpecialStringTok{f\textquotesingle{}PCGDP}\SpecialCharTok{\{}\NormalTok{year}\SpecialCharTok{\}}\SpecialStringTok{\textquotesingle{}} \ControlFlowTok{for}\NormalTok{ year }\KeywordTok{in}\NormalTok{ years]}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Plot Lorenz Curves for each decade}
\ImportTok{import}\NormalTok{ pandas }\ImportTok{as}\NormalTok{ pd}
...
In the study of spatial inequality, it is important to note that all the
measures mentioned above share a sixth property: \emph{spatial
invariance}. This means they are insensitive to the \emph{geographical}
distribution of the income values. The spatial invariance is
demonstrated in Figure~\ref{fig-invariance}, where we compare two
spatial distributions, one that is the actual distribution for 2000 and
one where we artificially permute the incomes randomly. Despite markedly
different spatial patterns, the income distributions summarized in the
histograms in the bottom row are identical. Since all of the inequality
measures mentioned only consider information about the statistical
distribution, each measure will take on the same value whether applied
to the spatial distribution on the left or right of the figure.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Generate a 2x2 grid}
\CommentTok{\# Choropleths in first row}
\CommentTok{\# Income histograms in second row}
...
respect the principle of symmetry or anonymity in a classic inequality
measure. This enables researchers to draw upon the wealth of knowledge
about the properties of classic inequality measures, but at the cost of
ignoring geography. In other words, these measures say a lot about
inequality in the statistical distribution of incomes but they are
silent on the spatial distribution of incomes.
...
These approaches are special cases of inequality decomposition. However,
we highlight a key distinction between those approaches that use the
concept of regional groupings to represent the geographical dimensions
of inequality, and those that take a more comprehensive approach to
introducing geography by considering the pair-wise spatial relationships
between observations. We label the former category as regional
inequality decomposition approaches, and the latter as spatial
...
To see how this works, we focus on the Theil inequality index
\citep{theil1967EconomicsInformation}. The Theil inequality index is a
measure of economic inequality derived from information theory,
capturing the extent to which a distribution deviates from perfect
equality. It is sensitive to differences across the distribution and can
be decomposed into ``within-group'' and ``between-group'' components to
analyze inequality at different levels.

More formally, let \(y_i\) be the income of unit \(i\), and \(s_i\)
represent the income share of unit \(i\) such that
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Produce 2x2 Grid}
\CommentTok{\# First row: spatial distribution and random distribution}
\CommentTok{\# Second row: another random distribution and density of}
\CommentTok{\# polarization values}
\NormalTok{fig, axs }\OperatorTok{=}\NormalTok{ plt.subplots(}\DecValTok{2}\NormalTok{, }\DecValTok{2}\NormalTok{)}
...
the top-right figure. An edge defines a pair of neighboring
states.\footnote{Two units are Queen neighbors if their borders share at
  least one vertex.}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Produce 2x2 Grid}
\CommentTok{\# First row: spatial distribution and neighbor graph}
\CommentTok{\# Second row: random distribution and density of}
\CommentTok{\# Spatial Gini values}
\NormalTok{fig, axs }\OperatorTok{=}\NormalTok{ plt.subplots(}\DecValTok{2}\NormalTok{, }\DecValTok{2}\NormalTok{)}
...
Spatial autocorrelation refers to the degree to which a spatial variable
is correlated with itself across geographical space. It measures whether
similar or dissimilar values of a variable tend to cluster together
spatially. Positive spatial autocorrelation indicates that similar
values (e.g., areas with high incomes) are located near each other,
while negative spatial autocorrelation suggests that dissimilar values
(e.g., areas with high and low incomes) are spatially proximate. Spatial
autocorrelation is crucial in studying spatial inequality because it
reveals the extent to which inequality is spatially patterned,
reflecting processes like segregation, clustering of poverty or wealth,
and the spatial diffusion of economic opportunities or disadvantages.

Identifying and quantifying spatial autocorrelation allows researchers
to understand the spatial dimensions of inequality, assess the
effectiveness of place-based policies, and model spatial processes more
accurately \citep{anselin1995LocalIndicators}. For example, regions with
high positive spatial autocorrelation of income inequality may require
targeted regional policies to address concentrated disadvantage. Without
considering spatial autocorrelation, analyses of inequality risk
overlooking critical spatial dependencies that shape social and economic
outcomes. Moreover, the presence of spatial autocorrelation implies that
inference on inequality measures that rely on the assumption of random
sampling may be misleading. This is because the autocorrelation violates
the assumption of independence underlying the random sampling
assumption.

While the spatial Gini decomposition does take spatial autocorrelation
into account, it does not allow for the exact additivity of within-group
and between-group inequality components. This occurs because the Gini
index depends on the degree of overlap in incomes among states from
different regions. Any overlap complicates the decomposition, as part of
the total inequality arises from the overlap itself, which cannot be
clearly attributed to either within-group or between-group inequality.
As a result, the decomposition requires a residual term to account for
the overlap. Since this residual term is difficult to interpret,
researchers rarely use the Gini index for regional inequality
decomposition.

\subsection{Weighted or Unweighted Inequality: Places versus
...
the Weighted Pen Parade under concept 2 shown in
Figure~\ref{fig-weighted-pengram} and comparing it with the Unweighted
Pen Parade from Figure~\ref{fig-pen2000}. The number of bars for each
state in the Weighted Pen Parade is proportional to the population of
that state. The logic behind the weighted spatial inequality is that
rather than having one representative observation (individual) from the
state with their level of income equal to the state per capita income,
now there are a proportional number of individuals from each state
having their level of income equal to the state's per capita
income.\footnote{A referee suggested that if data on individual incomes
  were available one could use a pentagram for representing within
  region/state/city inequality to complement
  Figure~\ref{fig-weighted-pengram}. In some cases, such as the
  Netherlands, access to micro level data may be possible under strict
  conditions.}
...
\caption{\label{fig-weighted-pengram}Populated Weighted Pen Parade}
...
Spatial inequality continues to attract the attention of researchers and
policy makers alike. This chapter has presented the key methodological
approaches available to researchers interested in analyzing spatial
disparities. By highlighting the challenges posed in adapting classical
inequality measures to the question of spatial inequality, the chapter
draws important distinctions between different types of inequality
decomposition approaches based on their treatment of space. These
methods reveal the spatial dimensions of inequality, particularly the
extent to which income disparities are geographically clustered.
...
Linking the spatial inequality measures to potential policy
interventions offers some interesting possibilities. For example, the
distinction between inter-regional (between-region) and intra-regional
(within-region) inequality in the decomposition may help researchers and
policymakers design such interventions in a tailored fashion. If
between-region inequality dominates, there may be a strong case for
place-based policies. Conversely, if the within-region inequality
component accounts for the majority share, more nationally oriented
policies, such as federal tax laws, may be more appropriate.
